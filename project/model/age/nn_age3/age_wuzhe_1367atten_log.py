...
def get_age_data():
    ...
    return data
'''
agefake=pd.read_csv('fakelabe/stack_agefake_agerate0.5_diff0.4_label2.csv')
#agefake=pd.read_csv('age_model/stack_agefake_agerate0.6_diff0.4_none1.csv')
#agefake=pd.read_csv('age_model/stack_agefake_agerate0.4_diff0.3_label2.csv')
data = agefake.sort_values(by=['user_id']).reset_index(drop=True)

truedata= get_age_data()
truedata = truedata.sort_values(by=['user_id']).reset_index(drop=True)

truedata_lab = truedata[truedata['age']!='-1']
label_age = to_categorical(truedata_lab['age'] - 1)
'''
data= get_age_data()
...
'''
emb1=np.load('512_w2c/w2v_512_creative_id_xuli95_user_id_win20iter15_emb.npy')
emb3=np.load('512_w2c/w2v_512_advertiser_id_xuli95_user_id_win20iter15_emb.npy')
emb6=np.load('512_w2c/w2v_512_ad_id_xuli95_user_id_win20iter15_emb.npy')
'''
...
#feature = pd.concat([f4,f5,f6], axis=1, sort=False)
#del f4,f5,f6
...
def model_conv(emb1, emb3, emb6,emb7,num_feature_input,len_id_input): #emb3,  , num_feature_input
    ...
    #x11 = concatenate([x1,x3,x6,x7], axis=-1 ) 
    ...
    '''
    #x11 = concatenate([x1,x3,x6] , axis=-1) 
    x12 = Dropout(0.4)(Bidirectional(CuDNNLSTM(256, return_sequences=True))(x12))       
    x12 = Dense(256, activation='relu')(x12)
    semantic12 = TimeDistributed(Dense(128, activation="tanh"))(x12)
    merged_12 = Lambda(lambda x: K.max(x, axis=1), output_shape=(128,))(semantic12)
    merged_12_avg = Lambda(lambda x: K.sum(x, axis=1), output_shape=(128,))(semantic12)
    '''
    #x = Dropout(0.4)(Bidirectional(CuDNNLSTM(320, return_sequences=True))(x11))       
...
def dataloader(test_index):
    ...
    x7_va = np.array(train_input_7)[test_index]#########
    ...
    x77_va =np.array(train_input_77)[test_index]######### 
    
    x5_va = np.array(train_input_5)[test_index]
    x9_len_va =np.array(train_input_len)[test_index]
    y_va =label[test_index]
    gc.collect()
    return x1_va, x3_va,x6_va, x5_va,x11_va, x33_va,x66_va,y_va  ,x7_va,x77_va,x9_len_va
...
filepath = "age_model/nn_v2_lstm_1367atten1_0.h5" 
model_age.load_weights(filepath)
x1_va, x3_va,x6_va, x5_va,x11_va, x33_va,x66_va ,y_va ,x7_va,x77_va,x9_len_va =dataloader(num0_test)

oof_pred[num0_test] = model_age.predict([x1_va, x3_va,x6_va,x7_va, x5_va,x9_len_va],batch_size=1024,verbose=1)###2048 
sub += model_age.predict([test_input_1, test_input_3,test_input_6,test_input_7,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits##   
oof_pred_da[num0_test] = model_age.predict([x11_va, x33_va,x66_va,x77_va, x5_va,x9_len_va],batch_size=1024,verbose=1)
sub_da += model_age.predict([test_input_11, test_input_33,test_input_66,test_input_77,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits
    
filepath = "age_model/nn_v2_lstm_1367atten1_1.h5" 
model_age.load_weights(filepath)
x1_va, x3_va,x6_va, x5_va,x11_va, x33_va,x66_va ,y_va ,x7_va,x77_va,x9_len_va=dataloader(num1_test)

oof_pred[num1_test] = model_age.predict([x1_va, x3_va,x6_va,x7_va, x5_va,x9_len_va],batch_size=1024,verbose=1)###2048
sub += model_age.predict([test_input_1, test_input_3,test_input_6,test_input_7,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits##
oof_pred_da[num1_test] = model_age.predict([x11_va, x33_va,x66_va,x77_va, x5_va,x9_len_va],batch_size=1024,verbose=1)
sub_da += model_age.predict([test_input_11, test_input_33,test_input_66,test_input_77,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits
###########################


filepath = "age_model/nn_v2_lstm_1367atten1_2.h5" 
model_age.load_weights(filepath)
x1_va, x3_va,x6_va, x5_va,x11_va, x33_va,x66_va ,y_va ,x7_va,x77_va,x9_len_va=dataloader(num2_test)

oof_pred[num2_test] = model_age.predict([x1_va, x3_va,x6_va,x7_va, x5_va,x9_len_va],batch_size=1024,verbose=1)###2048
sub += model_age.predict([test_input_1, test_input_3,test_input_6,test_input_7,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits##
oof_pred_da[num2_test] = model_age.predict([x11_va, x33_va,x66_va,x77_va, x5_va,x9_len_va],batch_size=1024,verbose=1)
sub_da += model_age.predict([test_input_11, test_input_33,test_input_66,test_input_77,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits
 ###########################   
    
filepath = "age_model/nn_v2_lstm_1367atten1_3.h5" 
model_age.load_weights(filepath)
x1_va, x3_va,x6_va, x5_va,x11_va, x33_va,x66_va ,y_va ,x7_va,x77_va,x9_len_va=dataloader(num3_test)

oof_pred[num3_test] = model_age.predict([x1_va, x3_va,x6_va,x7_va, x5_va,x9_len_va],batch_size=1024,verbose=1)###2048
sub += model_age.predict([test_input_1, test_input_3,test_input_6,test_input_7,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits##
oof_pred_da[num3_test] = model_age.predict([x11_va, x33_va,x66_va,x77_va, x5_va,x9_len_va],batch_size=1024,verbose=1)
sub_da += model_age.predict([test_input_11, test_input_33,test_input_66,test_input_77,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits
###########################

filepath = "age_model/nn_v2_lstm_1367atten1_4.h5" 
model_age.load_weights(filepath)
x1_va, x3_va,x6_va, x5_va,x11_va, x33_va,x66_va ,y_va ,x7_va,x77_va,x9_len_va=dataloader(num4_test)

oof_pred[num4_test] = model_age.predict([x1_va, x3_va,x6_va,x7_va, x5_va,x9_len_va],batch_size=1024,verbose=1)###2048
sub += model_age.predict([test_input_1, test_input_3,test_input_6,test_input_7,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits##
oof_pred_da[num4_test] = model_age.predict([x11_va, x33_va,x66_va,x77_va, x5_va,x9_len_va],batch_size=1024,verbose=1)
sub_da += model_age.predict([test_input_11, test_input_33,test_input_66,test_input_77,test_input_5,test_input_len],batch_size=1024,verbose=1)/skf.n_splits
...
np.save("age_model/wuzhe_submission_nn_age3_wuzhe_256wei_train4_7_atten_relr_{}.npy".format(np.mean(score1)), oof)
...
np.save("age_model/wuzhe_submission_nn_age3_wuzhe_256wei_train4_7_atten_relr_dastr_{}.npy".format(np.mean(score2)), oof1)