...
#输入3 id和tfidf count 特征 通过lstm 网络得到结果，用了不同顺序的序列进行数据增强
...
print("###"*35)
gc.collect()
filepath = "age_model/nn_v1_atten2020_5idcode_%d.h5" % count
...
del  train_input_1, train_input_3 ,train_input_6
gc.collect()
...
del  train_input_11, train_input_33 ,train_input_66
gc.collect()   
...
del  x1_va, x3_va,x6_va ,x11_va, x33_va,x66_va 
gc.collect()
...
del x5_tr, x5_va,y_tr,y_va
gc.collect()
hist = model_age.fit(  [da_x1_tr, da_x3_tr,da_x6_tr, da_x5_tr],     #
                         da_y_tr, batch_size=1024, epochs=50,  #4096
                          validation_data=([da_x1_va, da_x3_va, da_x6_va, da_x5_va], da_y_va), #  validation_data=([x1_va, x5_va], y_va),
